...
def get_distance(x, y, imagePoints):
    ...
    objectPoints = np.array([[33.6,75,0], #33
                            [33.6,85,0],
                            [23.6,85,0], #23
                            [23.6,75,0],],dtype = 'float32')
...
def hit(offset_x,offset_y):
    ...
    if length > 80 and length < 270:
        ...
        dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, offset_x, offset_y, -20, -110, isQueued = 1)

        dType.SetJOGCmd(api, 1, 2, 1)
        dType.SetWAITCmd(api, 300, 1)
        dType.SetJOGCmd(api, 1, 0, 1)

        dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, 200, 0, -20, -100, isQueued = 1)
